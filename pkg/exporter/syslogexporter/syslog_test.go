...
func TestFormatRFC5424(t *testing.T) {
	...
	msg := map[string]any{"timestamp": "2003-08-24T05:14:15-07:00", "appname": "myproc", "facility": 20,
		"hostname": "192.0.2.1", "log.file.name": "syslog", "message": "It's time to make the do-nuts.",
		"priority": 165, "proc_id": "8710", "version": 1}
	expected := "<165>1 2003-08-24T05:14:15-07:00 192.0.2.1 myproc 8710 - " +
	...
	msg2 := map[string]any{"timestamp": "2003-10-11T22:14:15Z", "appname": "evntslog", "facility": 20,
	...
	expected2 := "<165>1 2003-10-11T22:14:15Z mymachine.example.com evntslog - ID47 [exampleSDID@32473] " +
		"BOMAn application event log entry..."
	timeObj2, err := time.Parse(time.RFC3339, "2003-10-11T22:14:15.003Z")
	if err != nil {
		fmt.Print(err.Error())
	}
	assert.Equal(t, expected2, s2.formatRFC5424(msg2, timeObj2))
	...
	formattedMsg := s3.formatRFC5424(msg3, timeObj3)
...
func TestFormatRFC5424NoAddStructuredData(t *testing.T) {
	...
	timeObj, err := time.Parse(time.RFC3339, "2003-08-24T05:14:15.000003-07:00")
	assert.Equal(t, expected, s.formatRFC5424(msg, timeObj))
	if err != nil {
		fmt.Print(err.Error())
	}
	...
	timeObj1, err := time.Parse(time.RFC3339, "2003-10-11T22:14:15.003Z")
	if err != nil {
		fmt.Print(err.Error())
	}
	assert.Equal(t, expected2, s.formatRFC5424(msg2, timeObj1))
