...
func TestRandomTraceProcessing(t *testing.T) {
	...
	allIDs := map[pdata.TraceID]interface{}{}
...
func generateTraces(startingTraceId int, traceCount int, spanCount int, increaseMultiplier int) ([]pdata.TraceID, []pdata.Traces) {
	traceIds := make([]pdata.TraceID, traceCount)
	spanID := 0
...
func generateAndConsumeTraces(t *testing.T, tsp *cascadingFilterSpanProcessor) map[pdata.TraceID]interface{} {
	allIDs := map[pdata.TraceID]interface{}{}
...
func assertTracesEmpty(t *testing.T, tsp *cascadingFilterSpanProcessor, allIDs map[pdata.TraceID]interface{}) {
	for traceId := range allIDs {
		d, ok := tsp.idToTrace.Load(traceKey(traceId.Bytes()))
		if ok {
			v := d.(*sampling.TraceData)
