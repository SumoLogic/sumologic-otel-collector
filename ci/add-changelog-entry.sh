...
function get_default_pr_number() {
    ...
    existing_pr_number=$(curl -s "https://api.github.com/repos/SumoLogic/sumologic-otel-collector/commits/${commit_hash}/pulls?per_page=1" | jq '.[0].number' 2>/dev/null)
    ...
    latest_pr_number=$(curl -s "https://api.github.com/repos/SumoLogic/sumologic-otel-collector/issues?state=all&per_page=1" | jq '.[0].number')
...
echo "" >> ".changelog/${PR_NUMBER}.${CHANGE_TYPE}.txt"
